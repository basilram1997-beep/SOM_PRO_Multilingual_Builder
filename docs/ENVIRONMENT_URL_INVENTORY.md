...
| Production  | `https://som-pro.pages.dev`  | Approved commercial Cloudflare Pages origin |
...
| Frontend production Dockerfile | `apps/frontend/Dockerfile.production` has `ARG VITE_API_URL=/api`.                                                                                                                       | Environment configuration.                                                               | Keep the default `/api` and allow override only through build/deploy configuration.                                                                                    |
...
| Closed   | `apps/frontend/Dockerfile.production`                              | Build arg now defaults to `/api`.                                           | Keep as-is unless production intentionally uses an external API host.                                                                         |
...
| P0       | `.env.production.example`, `apps/frontend/.env.production.example` | Historical production placeholders.                                          | Production web uses `APP_URL=https://som-pro.pages.dev`, `VITE_API_URL=/api`, and deploy-time API/license routing under the commercial origin or explicit HTTPS service URLs. |
...
- Treat `https://som-pro.pages.dev` as the commercial production web origin.
