...
export async function createInvitationNotification(
  ...
  const title =
    input.invitationType === "PERMISSION"
      ? `استئذان الطالب ${input.studentName}`
      : `استدعاء ولي أمر الطالب ${input.studentName}`;
  const message =
    input.invitationType === "PERMISSION"
      ? `يرجى السماح للطالب ${input.studentName} من صف ${input.className} بالاستئذان بتاريخ ${input.date} عند الساعة ${input.time}. السبب: ${input.reason}${input.note ? `، ملاحظة: ${input.note}` : ""}.`
      : `يرجى حضور ولي أمر الطالب ${input.studentName} من صف ${input.className} إلى المدرسة بتاريخ ${input.date} عند الساعة ${input.time}. السبب: ${input.reason}${input.note ? `، ملاحظة: ${input.note}` : ""}.`;
