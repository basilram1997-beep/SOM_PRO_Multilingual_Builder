...
test("commercial install database checks are explicit and local-only by default", () => {
  ...
  const complianceDoc = readFileSync("../../docs/COMPLIANCE_AND_DESTRUCTIVE_TESTING.md", "utf8");
  const externalReviewPack = readFileSync("../../docs/EXTERNAL_REVIEW_PACK_AR.md", "utf8");
  const retentionPolicy = readFileSync("../../docs/RETENTION_AND_DELETION_POLICY.md", "utf8");
  const decisionLog = readFileSync("../../docs/DECISION_LOG_TEMPLATE.md", "utf8");
  ...
  const releaseCheck = readFileSync("../../scripts/runtime/release-check.js", "utf8");
  ...
  const operatorHealthTypes = readFileSync("../frontend/src/features/operatorHealth/operatorHealthTypes.ts", "utf8");
  const operatorHealthPage = readFileSync("../frontend/src/pages/settings/OperatorHealthPage.tsx", "utf8");
  const stagingEnv = readFileSync("../../.env.staging.example", "utf8");
  const backendStagingEnv = readFileSync(".env.staging.example", "utf8");
  const frontendStagingEnv = readFileSync("../frontend/.env.staging.example", "utf8");
  ...
  assert.match(stagingEnv, /VITE_SOM_SHOW_OPERATOR_HEALTH=true/);
  assert.match(stagingEnv, /SOM_ENABLE_OPERATOR_HEALTH=true/);
  assert.match(stagingEnv, /SOM_AUTO_BACKUP_INTERVAL_HOURS=24/);
  assert.match(stagingEnv, /SOM_REDUNDANCY_MODE=single-region/);
  assert.match(stagingEnv, /SOM_REPLICA_DATABASE_URL=postgresql:\/\/replica_user/);
  assert.match(backendStagingEnv, /SOM_ENABLE_OPERATOR_HEALTH=true/);
  assert.match(backendStagingEnv, /SOM_REDUNDANCY_MODE=single-region/);
  assert.match(frontendStagingEnv, /VITE_SOM_SHOW_OPERATOR_HEALTH=true/);
