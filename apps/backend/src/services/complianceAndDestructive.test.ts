...
test("compliance readiness keeps privacy, legal, and commercial controls documented", () => {
  ...
  has(externalReviewPack, /external privacy, security, and commercial review/i, "external review pack must be explicit");
  has(externalReviewPack, /ready for external review and approval/i, "external review pack must say review-ready");
  has(retentionPolicy, /explicit, logged, school-scoped/i, "retention policy must keep deletion school-scoped and logged");
  ...
  has(privacyRequirements, /deployment, retention, deletion, and export rules/i, "ownership boundary must stay explicit");
...
test("destructive and compliance-sensitive routes remain confirmation gated and school scoped", () => {
  ...
  has(classesRoutes, /teacherLessonToday\.deleteMany\(\{ where: \{ schoolId, classId \} \}\)/, "class delete must clear lessons");
  has(classesRoutes, /studentGradeEntry\.deleteMany\(\{ where: \{ schoolId, classId \} \}\)/, "class delete must clear grades");
  has(classesRoutes, /student\.deleteMany\(\{ where: \{ schoolId, classId \} \}\)/, "class delete must clear students");
  has(classesRoutes, /schoolClass\.delete\(\{ where: \{ id: classId \} \}\)/, "class delete must end with the class row");
  has(classesRoutes, /P2003/, "class delete must still guard foreign-key conflicts");

  has(teachersRoutes, /dailyTeacherStatus\.deleteMany\(\{ where: \{ schoolId, teacherId \} \}\)/, "teacher delete must clear status rows");
  ...
  has(teachersRoutes, /teacherAssignment\.deleteMany\(\{ where: \{ schoolId, teacherId \} \}\)/, "teacher delete must clear assignments");
  has(teachersRoutes, /teacher\.delete\(\{ where: \{ id: teacherId \} \}\)/, "teacher delete must end with the teacher row");
  has(teachersRoutes, /P2003/, "teacher delete must still guard foreign-key conflicts");

  has(reportsRoutes, /privacyWarningAccepted: z\.literal\(true\)/, "report exports must require privacy acknowledgement");
  has(requestProtections, /rejectSchoolContextOverride/, "request protections must reject school override attempts");
  has(appSource, /requirePermission\("manageSettings"\), securityIncidentsRouter/, "security routes must remain permission gated");
  has(appSource, /requirePermission\("manageSettings"\), auditLogsRouter/, "audit log routes must remain permission gated");
});
